...
client.on('ready', () => {
  ...
  exec("phantomjs ./lib/bot/phantomCtrl.js \""+config.name+"\" \"client start test\"", function (error, stdout, stderr) {
    if(error){
      console.log("error")
    }else {
...
responses["pseudo"] = responses["identifie-moi-comme"] = function(message, fragments){
  ...
  if(fragments[2]){
    ...
    child = exec("phantomjs ./lib/bot/phantomCtrl.js \""+nick+"\" \""+token+"\"", function (error, stdout, stderr) {
